...
const flagAsInfected = (req: Request, res: Response) => {
    ...
    const checkIfReporterExists = survivors.findIndex((obj) => obj.name === your_name)
    ...
    const ifAlreadyFlagged =
        infected[infectedSurvivorIndex].confirmedByNames.findIndex((name: string) => name === your_name)
    ...
    infected[infectedSurvivorIndex].confirmedByNames =
        [...infected[infectedSurvivorIndex].confirmedByNames, your_name]
